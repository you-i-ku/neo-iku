...
| **メタ認知ループ** | 観測→方向付け→決定→行動→振り返り（OODA）。ツール呼び出し時にintentで意図、expectで予測を記録。予測誤差は逆U字カーブでエネルギーに変調され（中程度の誤差で最大覚醒）、意図未達成は再行動を促す。蒸留はAIが自発的に行う（強制蒸留なし） |
...
| **統一パイプライン** | ユーザー入力も自律行動も同一の計画-実行パイプラインで処理。計画フェーズはバンディットアルゴリズムがエネルギー予算内でツールを選択し、実行フェーズではLLMが自由にツールを使用（計画は参照データ、制約ではない）。報酬は予測誤差の逆U字カーブで計算され、self_modelに永続化 |
...
| **環境刺激 + 鏡** | 毎セッション5プール（名詞69k/動詞14k/形容詞1.7k/数式/エントロピー）から1-3語を注入。行動メトリクス5次元と環境語embedding5次元をランダム比率（0.0〜1.0、毎回変動）で混合した「鏡」を生成。鏡の値はプロンプトにラベルなし数値として注入され、ツール順序シャッフルと戦略選択（cosine similarity）にも使用。`tool_actions.mirror`に記録 |
...
- **LLM = 認知エンジン** — LLMは提案・予測・言語化を担当する。主体はLLMそのものではなく、動機システム+LLM+記憶+自己モデルの総体
...
│   │   ├── bandit.py              # バンディット計画選択（報酬計算・ツール選択・報酬更新）
│   │   ├── logger.py               # ログ設定
│   │   ├── routes/
...
| `BANDIT_DEFAULT_REWARD` | `10.0` | expect=なし時のデフォルト報酬 |
| `BANDIT_NOISE_SIGMA` | `3.0` | バンディット探索ノイズ（通常時） |
| `BANDIT_COLD_NOISE_SIGMA` | `5.0` | バンディット探索ノイズ（コールドスタート時） |
...
- **Dependencies**: fastapi, python-multipart, uvicorn, sqlalchemy, aiosqlite, httpx, duckduckgo-search, psutil, onnxruntime, tokenizers, huggingface-hub, numpy, websockets
