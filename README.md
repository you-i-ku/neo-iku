...
- **内発的動機** — I/Oイベント（ユーザーメッセージ、ツール実行、予測誤差等）をシグナルとして蓄積し、AI自身が定義したルール（weights/threshold/decay）でエネルギーを計算。閾値（デフォルト=全action_costsの合計）を超えると自律行動が発火する（タイマーではなく内発的きっかけ）。ユーザー入力も即応答せずシグナルとして蓄積し、動機サイクルで処理
...
- **計画-実行分離** — 自律行動時にLLMがまずツール順序を計画し、各ツールを1つずつ実行。前のツール結果を見てから次を呼ぶため、ハルシネーションを抑制。チャットは従来の1アクション制のまま
- **環境刺激注入** — 自律サイクルごとにランダム確率で環境刺激（語彙のカケラ、エントロピーノイズ）をシグナルとして注入。AIのツールでは得られない外界の揺らぎのみ（自律性を奪わない設計）。確率自体も毎回ランダムに変動
- **蒸留モニタリング** — 自律度タブで蒸留ログを閲覧可能。セッションごとのツール実行履歴、予測の有無、一致/不一致、現在の原則リストを可視化
...
| `/api/distillation-log` | GET | 蒸留ログ（?limit=20&offset=0。セッション別ツール実行+予測+原則） |
...
| `MOTIVATION_DEFAULT_THRESHOLD` | `None` | 動機エネルギーの発火閾値（None=コスト平均×PLAN_MAX_TOOLSを動的計算。AIがルールで上書き可） |
...
| `PLAN_EXECUTE_ENABLED` | `True` | 自律行動で計画-実行分離を使用するか |
| `PLAN_MAX_TOOLS` | `5` | 計画フェーズの最大ツール数 |
| `ENV_STIMULUS_ENABLED` | `True` | 環境刺激注入を有効にするか |
| `ENV_STIMULUS_PROBABILITY` | `0.3` | 自律サイクルごとの環境刺激発生確率 |
...
| `get_system_metrics` | 自プロセス・データサイズ・動機状態の数値を返す（AI自身のスコープ） | 不要 |
...
- **統一入力モデル**: ユーザー入力を環境刺激と同列に扱い、全てautonomousベースにする（チャット/自律の区別を廃止）
- **デュアルパイプライン**: 2本のパイプラインで並行処理（自律思考とユーザー応答の同時実行）。ローカルLLMのメモリ制約解消後に実装
