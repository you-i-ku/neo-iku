...
チャット時だけ存在するのではなく「ここに実存する」AIを追求する実験的プロジェクト。初期知識・目標・性格を一切与えず、内発的動機とメタ認知ループだけで行動が立ち上がるかを検証する。
...
L1-2が行動を間接的に方向づける可能性は認識しており、その境界は継続的に検証する。しかしながらL3はAI自身が自律的に獲得する領域として、ここには一切触れない。
...
│       ├── style.css
│       └── app.js
├── data/                           # 自動生成（SQLite DB + self_model.json + IPAdic辞書 + personas/）
...
- **ログ** — サーバーログ（レベルフィルタ付き）
- **自律度** — 行動レポート（13指標 + スパークライン推移グラフ）、蒸留ログ（リアルタイム更新）、Ablation実験
