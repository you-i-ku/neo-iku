...
| **メタ認知ループ** | 観測→方向付け→決定→行動→振り返り（OODA）。ツール呼び出し時にintentで意図、expectで予測を記録。予測誤差は逆U字カーブでエネルギーに変調され（中程度の誤差で最大覚醒）、意図未達成は再行動を促す。蒸留はAIが自発的に行う（強制蒸留なし）。セッション要約が`self_model.session_log`に最大10件保持され、全プロンプトに注入（認知連続性） |
...
│  ツールエラーのシグナル発火                   │
│  習熟検出（予測精度高→探索シグナル発火）       │
│  セッション要約 → self_model.session_logに保存 │
...
| `SESSION_LOG_MAX` | `10` | セッション要約の最大保持件数（超過分はsession_archiveに圧縮退避） |
