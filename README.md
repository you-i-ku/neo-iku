...
| **統一パイプライン** | ユーザー入力も自律行動も同一の計画-実行パイプラインで処理。まずツール計画を立て、ツールごとに個別実行（結果を次に引き継ぎ） |
...
| **自律度モニタ** | 行動ログ・蒸留ログ・自律性指標レポートをUIで可視化。エネルギー駆動率・ツール使用分布・エントロピー推移・予測精度推移・エネルギー効率・自己モデル変化速度・セッション長推移をスパークラインで計測。自律行動セッションを`data/action_logs/`に日次Markdownで自動出力 |
