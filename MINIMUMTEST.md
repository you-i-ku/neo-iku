...
固定タイマーから環境応答型に変更済み（3層構造）。

```
感覚層（1Hz）: 電脳気候4要素を計測 → rolling Z-score正規化
蓄積層（常時）: pressure = pressure * decay + env_delta + clock_base + e3_delta
認知層（閾値起動）: LLMサイクル（propose → execute）→ E2フィードバックでpressureリセット
```

- **安定時**: 約30秒に1サイクル（clock_base=0.1, threshold=12.0のデフォルト時）
- **環境が活発なとき**: より高頻度に発火
- **E2フィードバック**: `pressure = pressure * post_fire_reset + (e2 - 0.5) * scale`
  → 達成度が高いほど次サイクルが早く来る
- **E3内的駆動**: `e3_delta = max(0, 0.5 - last_e3) * e3_pressure_scale`
  → 前回の予測が外れた（E3低い）ほど次サイクルが早く来る（Active Inference的）
  → スケールは環境刺激主要素（weight=0.3）と同格（max寄与=0.3）に設定
...
```json
{
  ...
  "last_e3": 0.5                // 直前サイクルの予測精度（e3_delta計算に使用）
}
```
...
### 実装済み: 情報空間の環境（電脳気候）と自律駆動アーキテクチャ
